...
def gatherCookieJar():
    ...
    login = (requests.post(
        "https://amblesideonline.org/forum/member.php?action=login", data=DATA)
             )  #Gathers cookies nessecary for login.
...
def getPage(tid, pagenumber):
    page = requests.get("https://amblesideonline.org/forum/showthread.php?tid="
                        + str(tid) + "&page=" + str(pagenumber),
...
def savePage(tid, pagenumber):
    pagefile = open("thread" + str(tid) + "page" + str(pagenumber) + ".html",
                    "w")
...
def getTimes1Page():
    timesoup = BeautifulSoup(
        str(psoup.find_all(
            "div", class_="float_left smalltext")), 'lxml')
    return timesoup
...
def writeThread(tid, pagenumber, pages):
    global writef
    writef = open(
        str(tid) + "pages" + str(pagenumber) + "-" +
        str(pagenumber + pages - 1) + "___" + str(time.localtime()[3]) + ":" +
        str(time.localtime()[4]) + "." + str(time.localtime()[5]) + "___" +
        str(time.localtime()[1]) + "-" + str(time.localtime()[2]) + "-" +
        str(time.localtime()[0]) + ".csv", 'a')
    for i in range(pages):
        openToParse(tid, pagenumber + i)
        global posts
        global authors
...
def writePageCustom():
    for j in range(10):
        ...
        author = re.sub('\\\\n', '', re.search('\\\\n.*\\\\n\\\\n\\\\n',
                                               authors[j]).group(0))
        post = re.sub('\\\\n', '\n', re.sub(
            '\\\\t|\\\\r', '', posts.find_all("div")[j].get_text()))

        writef.write(
            "        At " + time + author + " wrote:\n" + post +
...
def writePageWA():
    for j in range(10):
        ...
        writef.write(time + "\t" + author + "\t" + post[1] + "\n")
...
print("Downloading " + str(pagerange) + " pages of thread " + str(threadnum) +
      " starting at page " + str(pagenum) + ".")
...
towriteornottowrite = raw_input(
    "Do you want the thread in a .CSV/spreadsheet (seperator is tab)?  y/N:  ")
...
towritecustomornottowritecustom = raw_input(
    "Do you want the thread written to an easily readable/editable text file?  y/N:  ")
if (towritecustomornottowritecustom == "Y" or
        towritecustomornottowritecustom == "y"):
    writeThreadCustom(threadnum, pagenum, pagerange,
                      str(raw_input("What should the file be called?  ")))

if (threadnum == 468 or threadnum == 19185):
    waconfirm = raw_input(
        "Do you want the first word in each post in a file?  y/N:  ")
    if (waconfirm == "Y" or waconfirm == "y"):
        writeThreadWA(threadnum, pagenum, pagerange,
                      str(raw_input("What should the file be called?  ")))
